    ...
    <section class="content-header">
      <?php 
       if(isset($_GET['res'])){
        ...
        $id=$_GET['id'];
        $result = $conn -> query("SELECT id, noticias, data, professor, aluno FROM noticias WHERE id = '$id'");
        $row = $result->fetch(PDO::FETCH_OBJ);
        ?>
      
      <form class="form-check-label" method="post" action="../model/update_noticias.php">
      <input type="hidden" name="id" value="<?php echo $row -> id?>">
	   <p style="font-size: 40px;">Noticia</p>
       <textarea style="width: 100%; font-size: 20px;" rows="10" cols="40" name="noticia" maxlength="800000"   value=""  required=""><?php echo $row-> noticias?></textarea>	
       <input style="float: right;" type="date" name="data" value="<?php echo $row-> data?>" required=""><samp style="float: right; margin-right: 20px;font-size: 20px">Data da noticia  </samp>
       <br>
       <br>
       <br>
       <p style="font-size: 20px">Permição para ler a notica.</p>
       <?php 
       if($row-> professor == 1){
       echo '<input style="width: 20px; height: 20px;" type="checkbox" name="professor" checked />
       <span style="font-size: 20px">Professor</span><br>';
       
       }else{
        echo '<input style="width: 20px; height: 20px;" type="checkbox" name="professor"/>
       <span style="font-size: 20px">Professor</span><br>';
       }
       if($row-> aluno == 1){
       echo '<input style="width: 20px; height: 20px;" type="checkbox" name="aluno" checked />
       <span style="font-size: 20px">Aluno</span><br>';
       
       }else{
        echo '<input style="width: 20px; height: 20px;" type="checkbox" name="aluno"/>
       <span style="font-size: 20px">Aluno</span><br>';
       }?>
  	   
       <samp>A noticia sera visualizada de acordo com a permissão</samp>

  		
  		<br>
  		<button onclick="confirm('Enviar noticia') " class="btn btn-primary" style="float: right; font-size: 20px; width: 10% ; margin-top: -20px">envia</button>
