...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
     ...
     <i style="font-size: 80px" class="fa fa-id-card-o" aria-hidden="true"><small style="font-size: 60px">
        Alterar Aluno
      ...
      <?php 
        ...
        $result = $conn -> query("SELECT id, nome, matricula, id_curso, cpf, telefone, email, id_turma FROM aluno WHERE id = '$id'");
       ...
       <form method="post" action="../model/update_aluno.php">
        <div class="form-group">
          <input type="hidden" name="id" value="<?php echo $row -> id?>">
          <label style="font-size: 20px" for="exampleInputEmail1">Nome*</label>
          <input style="width: 50%; font-size: 20px" type="text" class="form-control" name="nome" placeholder="Nome" required value="<?php echo $row-> nome ?>">
          
        </div>
         <div class="form-group">

          <label style="font-size: 20px" for="exampleInputEmail1">Matricula*</label>
          <input style="width: 50%; font-size: 20px" maxlength="8" type="text" class="form-control" name="matricula" placeholder="matricula" required value="<?php echo $row-> matricula ?>">

          
          
        </div>
        <label style="font-size: 20px" for="exampleInputEmail1">Selecione o curso*</label><br>
          <select style="color: #000; width:300px; height: 30px; width: 50%; font-size: 17px;" id="curso" name="id_curso" class="form-text text-muted" onchange="buscar_minicursos()" required>
                  <?php include_once '../model/conn.php';
                       $result3 = $conn -> query("SELECT id, nome FROM curso");
                       $result4 = $conn -> query("SELECT id, nome FROM curso WHERE id = '$id_curso'");
                       $row4 = $result4->fetch(PDO::FETCH_OBJ);
                        if ($result) {
                           echo '<option style="color: black; " required value="'.$row4 -> id.'">'.$row4 -> nome.'</option>';
                          while($row3 = $result3->fetch(PDO::FETCH_OBJ)){
                            
                            echo '<option style="color: black; " value="'.$row3 -> id.'">'.$row3-> nome.'</option>';
                          }
                        }   
                  ?>
          ...
          <label style="font-size: 20px" for="exampleInputEmail1">Diciplina ministrada*</label><br>
          <select style="color: #000; width:300px; height: 30px; width: 50%; font-size: 17px;" name="id_turma" id="minicurso" class="form-text text-muted" name="id_disciplina" required>
            <?php  
            $result5 = $conn -> query("SELECT id, nome FROM turma WHERE id = '$id_turma'");
            $row5 = $result5->fetch(PDO::FETCH_OBJ);
            ?>
             
          
            <option value="<?php echo $row5 -> id?>"><?php echo $row5 -> nome ?></option>
        ...
        <div class="form-group">
          ...
          <label style="font-size: 20px" for="exampleInputEmail1">CPF*</label>
          <input style="width: 50%; font-size: 20px" type="text" maxlength="11" class="form-control" name="cpf" placeholder="CPF" required value="<?php echo $row-> cpf ?>"/>
          
        </div>
        <div class="form-group">

          <label style="font-size: 20px" for="exampleInputEmail1">Email</label>
          <input style="width: 50%; font-size: 20px" type="email" class="form-control" name="email" placeholder="EX.: exemplo@exmple.com" value="<?php echo $row-> email ?>">
        ...
        <div class="form-group">
          ...
          <label style="font-size: 20px" for="exampleInputEmail1">Telefone</label>
          <input style="width: 50%; font-size: 20px" type="text" id="telefone" maxlength="15" class="form-control" name="telefone" placeholder="(DD) 00000-0000" value="<?php echo $row-> telefone ?>">
         
         
        </div>
        <button style="width: 50% ; font-size: 20px" type="submit" onclick="return confirm('Deseja Alterar o aluno')" class="btn btn-primary">Alterar</button>
      </form>
