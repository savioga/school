...
<head>
  ...
  <title>Relatorio de Ministração</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1 style="font-size: 60px">
        Relatorio de Ministração de Aulas
       ...
       if(isset($_GET['resut'])){
        ...
        if ($s=='error') {
            echo '<div style="width: 70%; padding: 6px; font-size: 40px; margin-top: 10px" class="alert alert-danger" role="alert">
                 Erro! Não se Pode Apargar uma Ministração Relacionado!
                </div>';
            ...
            echo '<div style="width: 70%; padding: 6px; font-size: 40px; margin-top: 10px" class="alert alert-success" role="alert">
                 O Ministração de ID '.$s.' deletado com sucesso!
                </div>';
          }
      }   
          if(isset($_GET['re'])){
            ...
            echo '<div style="width: 70%; padding: 6px; font-size: 20px; margin-top: 10px" class="alert alert-danger" role="alert">
                Error! Professor ja ministra essa disciplina!
                </div>';
          }
          ?>
    ...
    include_once '../model/conn.php';
        ...
        $result = $conn -> query("SELECT id, id_curso, id_disciplina, id_professor FROM ministra");
        ...
        if ($result) {
          ...
          while($row = $result->fetch(PDO::FETCH_OBJ)){
                   ...
                   $result4 = $conn -> query("SELECT id, nome FROM disciplina WHERE id = '{$row-> id_disciplina}'");
                   $result5 = $conn -> query("SELECT id, nome FROM professor WHERE id = '{$row-> id_professor}'");   
                   ...
                   $row5 = $result5->fetch(PDO::FETCH_OBJ);
                    ...
                    echo  '<tr class="tr2">
                           ...
                           <td><h3 style="margin-left:  80px";><p>'.$row5 -> nome .'</p></h3></td>
                           <td><h3 style="margin-left:  80px;"><p>'.$row3 -> nome .'</p></h3></td>
                           <td><h3 style="margin-left:  80px;"><p>'.$row4 -> nome .'</p></h3></td>

                           <td><h3 style="margin-left: 5px;"><a href="alterar_ministracao.php?id='.$row -> id.'"><button title="Alterar" style="font-size: 30px; margin-top: -15px" onclick="return confirm('."'Deseja Alterar a Disciplina? '".')" type="submit" class="btn btn-primary"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a></h3></td>
                           <td><h3 style="margin-left: 5px;"><a href="../model/deletar_ministracao.php?id='.$row -> id.'"><button title="Excluir" style="font-size: 30px; margin-top: -15px" onclick="return confirm('."'Deseja deletar a ministraçao? '".')" type="submit" class="btn btn-danger"><i class="fa fa-trash" aria-hidden="true"></i></button></a></h3></td>
                        </tr>';
                        $cont++;
  ...
  ?>
    ...
    <?php 
    if($cont==0){
          echo '<p style="font-size: 30px">Nenhuma minidtrsção foi cadastrada!</p>';
        }
     ?> 
    </section>
