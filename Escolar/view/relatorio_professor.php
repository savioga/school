...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <?php  
       if(isset($_GET['resut'])){
        ...
        if ($s=='error') {
            echo '<div style="width: 70%; padding: 6px; font-size: 40px; margin-top: 10px" class="alert alert-danger" role="alert">
                 Erro! Não se Pode Apargar um Professor Relacionado!
                </div>';
            ...
            echo '<div style="width: 70%; padding: 6px; font-size: 40px; margin-top: 10px" class="alert alert-success" role="alert">
                 O Professor de ID '.$s.' deletado com sucesso!
                </div>';
          }
      }
       ...
       if(isset($_GET['re'])){
            ...
            echo '<div style="width: 70%; padding: 6px; font-size: 20px; margin-top: 10px" class="alert alert-danger" role="alert">
                Error! Já exite um Professor com esse CPF!
                </div>';
          } 
      ?>
      ...
      <table class="table">
            <tr class="tr1">
              ...
              <td><h2 style="margin-left: 80px;"><b>Formação</b><h2></td>
    ...
    include_once '../model/conn.php';
        ...
        $result = $conn -> query("SELECT id, nome, formacao, cpf, email, telefone FROM professor");
        
        if ($result) {
          //percorre os resultados via iteração
          while($row = $result->fetch(PDO::FETCH_OBJ)){
                  
                    $salvar='Deseja Alterar';
                    echo  '<tr class="tr2">
                           <td><b><h3 style="margin-left:  3"><p>'.$row -> id .'</p></b></h3></td> 
                           <td><h3 style="margin-left:  80px";><p>'.$row -> nome .'</p></h3></td>
                           <td><h3 style="margin-left:  80px;"><p>'.$row -> formacao .'</p></h3></td>
                           ...
                           <td><h3 style="margin-left: 5px;"><a href="alterar_professor.php?id='.$row -> id.'"><button title="Alterar" style="font-size: 30px; margin-top: -15px" onclick="return confirm('."'Deseja Alterar o Professor? '".')" type="submit" class="btn btn-primary"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a></h3></td>
                           <td><h3 style="margin-left: 5px;"><a href="../model/deletar_professor.php?id='.$row -> id.'"><button title="Excluir" style="font-size: 30px; margin-top: -15px" onclick="return confirm('."'Deseja deletar o Professor? '".')" type="submit" class="btn btn-danger"><i class="fa fa-trash" aria-hidden="true"></i></button></a></h3></td>
                        </tr>';
                        $cont++;
  ...
  ?>
      ...
      <?php  
      if($cont==0){
          echo '<p style="font-size: 30px">Nenhum professor foi cadastrado!</p>';
        }
      ?>
    </section>
