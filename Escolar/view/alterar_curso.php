     ...
     <i style="font-size: 80px" class="fa fa-graduation-cap" aria-hidden="true"><small style="font-size: 60px">
      ...
      <br>
       <?php 
        ...
        $result = $conn -> query("SELECT id, nome FROM curso WHERE id = '$id'");
        $row = $result->fetch(PDO::FETCH_OBJ);

       ?> 
      
      <form method="post" action="../model/update_curso.php">
        <div class="form-group">
          <input type="hidden" name="id" value="<?php echo $row -> id?>">
          <label style="font-size: 20px" for="exampleInputEmail1">Nome Do Curso*</label>
          <input style="width: 50%; font-size: 20px" type="text" class="form-control" name="nome" placeholder="Curso" value="<?php echo $row -> nome?>">
          <?php  
            if(isset($_GET['resu'])){
              $s = $_GET['resu'];
              if($s=='error'){
        ...
        <button style="width: 50% ; font-size: 20px" type="submit" onclick="return confirm('Deseja Salvar o Curso')" class="btn btn-primary">Salvar</button>
      </form>
