  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
     <i style="font-size: 80px" class="fa fa-clipboard" aria-hidden="true"><small style="font-size: 60px">
      ...
      ?>
      <form method="post" action="../model/salvar_aluno_disciplina.php">
        <input type="hidden" name="id_curso" value="<?php echo $curso?>">
        <input type="hidden" name="id_turma" value="<?php echo $turma?>">
         <input type="hidden" name="id_ministra" value="<?php echo $ministra?>">
        <label style="font-size: 20px" for="exampleInputEmail1">Selecione o aluno*</label><br>
        <select id="Curso" name="id_aluno" class="form-text text-muted" required>
                <?php include_once '../model/conn.php';
                     $result2 = $conn -> query("SELECT id, nome FROM aluno WHERE id_turma = '$turma'");
                     
                      if ($result2) {
                         echo '<option style="color: black;" value="">Selecione uma aluno</option>';
