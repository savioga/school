...
<head>
  ...
  <title>Relatorio de Noticias</title>
...
<div class="wrapper">
    ...
    <section class="content-header">
      ...
      <?php  
        if(isset($_GET['resut'])){
          $s = $_GET['resut'];
          if($s =='sucesso'){
            echo '<div style="width:70%; padding: 6px; font-size: 20px; " class="alert alert-success" role="alert">
                 O curso deletado com sucesso!
                </div>';
         }
       }
          if(isset($_GET['resu'])){
          $s = $_GET['resu'];
            ...
            echo '<div style="width: 70%; padding: 6px; font-size: 40px; margin-top: 10px; margin-left: 15px" class="alert alert-success" role="alert">
                 A noticia de ID '.$s.' deletado com sucesso!
                </div>';
          }
           if(isset($_GET['res'])){
          $s = $_GET['res'];
            ...
            echo '<div style="width: 70%; padding: 6px; font-size: 40px; margin-top: 10px; margin-left: 15px" class="alert alert-success" role="alert">
                 A noticia de ID '.$s.' Alterada com sucesso!
                </div>';
          }
      ...
      include_once '../model/conn.php';
          ...
          if ($result) {
            ...
            while($row = $result->fetch(PDO::FETCH_OBJ)){
                      ...
                      echo  '<div style="border: 2px solid #ccc; background:white;">
                      <table style="float: right; margin-right: 40px"><tr><td><h3 style="margin-left: 5px; margin-left:95%;"><a href="../model/deletar_noticia.php?id='.$row -> id.'"><button title="Excluir" onclick="return confirm('."'Deseja deletar o Curso? '".')" type="submit" class="btn btn-danger"><i class="fa fa-trash" ></i></button></a></h3> </td>
                       <td><h3 style="margin-left: 5px; margin-left:95%;"><a href="../view/alterar_noticia.php?id='.$row -> id.'"><button title="Editar" onclick="return confirm('."'Deseja Edita a noticia? '".')" type="submit" class="btn btn-primary"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a></h3> </td></tr></table>
                              <div>
                                <h1 style="margin-left: 5px">ID: '.$row-> id.'</h1>
                              </div>
                             
                              <div>
                                <h1 style="margin-left: 5px">Noticias:</h1>
                              
                                <h3 style="margin-left: 10px; margin-right: 10px">'.$row-> noticias.'<?h3>
                              </div>
                              <br>
                                 <right><h4 style=" margin-left: 90%">Data da noticia: '.$row-> data.'</h4></right>
                                 <h3  style="margin-left: 5px">Permissões</h3>
                                 ';
                                 if ($row-> professor==0) {
                                  echo '<p  style="margin-left: 5px">Professor: Não</p>';
                                }else{
                                  echo '<p style="margin-left: 5px">Professor: Sim</p>';
                                }if ($row-> aluno==0) {
                                  echo '<p style="margin-left: 5px">Aluno: Não</p>';
                                }else{
                                  echo '<p style="margin-left: 5px">Aluno: Sim</p>';
                                }
                            echo '</div>
                            <br>
                            <br>
  ...
  <?php 
   if($cont==0){
          echo '<p style="font-size: 30px">Nenhuma noticia foi lançada!</p>';
        }
   ?>
    </section>
