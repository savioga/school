     ...
     <i style="font-size: 80px" class="fa fa-clipboard" aria-hidden="true"><small style="font-size: 60px">
      ...
      ?>
       ...
       <?php 
        ...
        $result = $conn -> query("SELECT id, nome, id_curso FROM disciplina WHERE id = '$id'");
        $row = $result->fetch(PDO::FETCH_OBJ);
        $id_curso = $row -> id_curso;
       ?> 

      <form method="post" action="../model/update_disciplina.php">
        <div class="form-group">
          <input type="hidden" name="id" value="<?php echo $row -> id?>">
          <label style="font-size: 20px" for="exampleInputEmail1">Nome Da Disciplina*</label>
          <input id="Disciplina" type="text" class="form-control" name="nome" placeholder="Disciplina" value="<?php echo $row-> nome  ?> ">
          <?php  
            if(isset($_GET['resu'])){
              $s = $_GET['resu'];
              if($s=='error'){
                echo '</i><div>
                       <p style="color: red">Nome obrigatirio! </p>
                   </div>';
              }
        ...
        <select id="Curso" name="id_curso" class="form-text text-muted" required>
                <?php include_once '../model/conn.php';
                     $result2 = $conn -> query("SELECT id, nome FROM curso");
                     $result3 = $conn -> query("SELECT id, nome FROM curso WHERE id = '$id_curso'");
                     $row3 = $result3->fetch(PDO::FETCH_OBJ);
                      if ($result) {
                         echo '<option style="color: black; " value="'.$row3 -> id.'" disabled>'.$row3-> nome.'</option>';
                        while($row2 = $result2->fetch(PDO::FETCH_OBJ)){
                          
                          echo '<option style="color: black; " value="'.$row2 -> id.'">'.$row2-> nome.'</option>';
                        }
                      }   
                ?>
         ...
         <?php  
            if($s=='error'){
                echo '</i><div>
                       <p style="color: red">Selecio um Curso!</p>
                   </div>';
              }
         ?> 
         <br>
         <br>
        <button style="width: 50% ; font-size: 20px" type="submit" onclick="return confirm('Deseja Salvar a Disciplina')" class="btn btn-primary">Salvar</button>
      </form>
