  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
     <i style="font-size: 80px" class="fa fa-clipboard" aria-hidden="true"><small style="font-size: 60px">
      	...
      	if(isset($_GET['res'])){
			...
			if ($s=='error') {
				echo '<div style="width: 50%; padding: 6px" class="alert alert-danger" role="alert">
  						Error ao cadastra. Turma já cadastrada nesse curso!
					  </div>';
			}
			if($s=='sucesso'){
      ...
      ?>
      <form method="post" action="../model/salvar_turma.php">
        <div class="form-group">

          <label style="font-size: 20px" for="exampleInputEmail1">Nome Da Turma*</label>
          <input id="Disciplina" type="text" class="form-control" name="nome" placeholder="Disciplina">
          <?php  
            if(isset($_GET['resu'])){
              $s = $_GET['resu'];
              if($s=='error'){
        ...
        <select id="Curso" name="id_curso" class="form-text text-muted" required>
                <?php include_once '../model/conn.php';
                     $result = $conn -> query("SELECT id, nome FROM curso");
        			  echo '<option style="color: black; value="">Selecione Uma Opção</option>';
                      if ($result) {
