...
<head> 
  ...
  <title>Cadastrar Ministração</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
      	...
      	if(isset($_GET['res'])){
			...
			if ($s=='error') {
				echo '<div style="width: 50%; padding: 6px" class="alert alert-danger" role="alert">
  						Essse professor já ministra nessa diciplina
					  </div>';
			}
			if($s=='sucesso'){
    ...
    function buscar_minicursos(){
      ...
      <form method="post" action="../model/salvar_ministracao.php">
          <label style="font-size: 20px" for="exampleInputEmail1">Professor que ministrará a aluna*</label><br>
          <select style="color: #000; width:300px; height: 30px; width: 50%; font-size: 17px;"class="form-text text-muted" name="id_professor" required>
                  <?php include_once '../model/conn.php';
                       $result = $conn -> query("SELECT id, nome FROM professor");
                        if ($result) {
                           echo '<option style="color: black; " required value="">Seleciona Um Professor</option>';
        ...
        <br>
          <br>
        <button style="width: 50% ; font-size: 20px;" type="submit" onclick="return confirm('Deseja Salvar a Ministração')" class="btn btn-primary">Salvar</button>
      </form>
         </select> 
