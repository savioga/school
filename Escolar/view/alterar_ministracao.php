...
<head> 
  ...
  <title>Alterar Ministração</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
     <i style="font-size: 80px" class="fa fa-clipboard" aria-hidden="true"><small style="font-size: 60px">
        Alterar Disciplina
      	...
      	if(isset($_GET['res'])){
			...
			if ($s=='error') {
				echo '<div style="width: 50%; padding: 6px" class="alert alert-danger" role="alert">
  						Error ao cadastra. Selecione um curso!
					  </div>';
			}
			if($s=='sucesso'){
      ...
      ?>
       ...
       <?php 
        ...
        $result = $conn -> query("SELECT id, id_professor,id_disciplina, id_curso FROM ministra WHERE id = '$id'");
    ...
    function buscar_minicursos(){
      var curso = $('#curso').val();
      if(curso){
        var url = 'busca_d.php?curso='+curso;
      ...
      <form method="post" action="../model/update_ministracao.php">
        <div class="form-group">
          
        </div>
        <input type="hidden" name="id" value="<?php echo $row -> id?>">
        <label style="font-size: 20px" for="exampleInputEmail1">Selecione o Professor*</label><br>
        <select id="Curso" name="id_professor" class="form-text text-muted" required>
                <?php include_once '../model/conn.php';
                     $result1 = $conn -> query("SELECT id, nome FROM professor");
                     $result2 = $conn -> query("SELECT id, nome FROM professor WHERE id = '$id_professor'");
                     $row2 = $result2->fetch(PDO::FETCH_OBJ);
                      if ($result) {
                         echo '<option style="color: black; " value="'.$row2 -> id.'" disabled>'.$row2-> nome.'</option>';
                        while($row1 = $result1->fetch(PDO::FETCH_OBJ)){
                          
                          echo '<option style="color: black; " value="'.$row1 -> id.'">'.$row1-> nome.'</option>';
                        }
                      }   
                ?>
         ...
         <br>
         <br>
         <label style="font-size: 20px" for="exampleInputEmail1">O curso que irá ser ministrado a aluna*</label><br>
          ...
          <select style="color: #000; width:300px; height: 30px; width: 50%; font-size: 17px;" name="id_disciplina" id="minicurso" class="form-text text-muted" name="id_disciplina" required>
            <?php  
            $result5 = $conn -> query("SELECT id, nome FROM disciplina WHERE id = '$id_disciplina'");
         ...
         <br>
         <br>
        <button style="width: 50% ; font-size: 20px" type="submit" onclick="return confirm('Deseja Alterar a Ministração')" class="btn btn-primary">Salvar</button>
      </form>
