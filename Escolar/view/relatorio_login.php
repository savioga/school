...
<head>
  ...
  <title>Relatorio de Logins</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <?php  
        if(isset($_GET['resut'])){
          ...
          if ($s=='error') {
            echo '<div style="width: 70%; padding: 6px; font-size: 20px; " class="alert alert-danger" role="alert">
                 Erro! Não se pode apagar um curso relacionado!
                </div>';
          ...
          if(isset($_GET['r'])){
            ...
            echo '<div style="width: 70%; padding: 6px; font-size: 20px; margin-top: 10px" class="alert alert-success" role="alert">
                 O Curso de ID '.$s.' Alterado com sucesso!
                </div>';
          }
          ?>
        ...
        <div class="content">
      <table style="" class="table" id="table">
            <tr class="tr1">
              <td style="margin-left: 3px;"><h2><b>ID</b></h2></td>
              <td><h2 style="margin-left: 80px;"><b>Login</b><h2></td>
              <td><h2 style="margin-left: 80px;"><b>Senha</b><h2></td>
              <td><h2 style="margin-left: 80px;"><b>Professor</b><h2></td>
              <td><h2 style="margin-left: 80px;"><b></b></h2></td>
              <td><h2 style="margin-left: 80px;"><b></b></h2></td> 
              <tbody id="myTable"> 
    ...
    <?php
      ...
      include_once '../model/conn.php';
          ...
          $result = $conn -> query("SELECT id, login, senha, id_professor FROM usuario");
          
          if ($result) {
            //percorre os resultados via iteração
            while($row = $result->fetch(PDO::FETCH_OBJ)){
                     $result3 = $conn -> query("SELECT nome FROM professor WHERE id='{$row-> id_professor}'");
                    $row2 = $result3->fetch(PDO::FETCH_OBJ);

                      echo  '<tr class="tr2">
                             <td><b><h3 style="margin-left:  3"><p>'.$row -> id .'</p></b></h3></td> 
                             <td><h3 style="margin-left:  80px";><p>'.$row -> login .'</p></h3></td>
                             <td><h3 style="margin-left:  80px";><p>********</p></h3></td>
                             <td><h3 style="margin-left:  80px";><p>'.$row2 -> nome .'</p></h3></td>
                             <td><h3 style="margin-left:  80px";><p></p></h3></td>

                             <td><h3 style="margin-left: 5px;"><a href="../model/deletar_login.php?id='.$row -> id.'"><button title="Excluir" style="font-size: 30px; margin-top: -15px" onclick="return confirm('."'Deseja deletar o Curso? '".')" type="submit" class="btn btn-danger"><i class="fa fa-trash" aria-hidden="true"></i></button></a></h3></td>
                          </tr>';
                          $cont++;
  ...
  <?php  
  if($cont==0){
          echo '<p style="font-size: 30px">Nenhum login foi cadastrado!</p>';
        }
  ?>
    </section>
