    ...
    <section class="content-header">
      ...
      <script>
        function buscar_minicursos(){
          var curso = $('#disciplina').val();
          if(curso){
            var url = 'busca_b.php?curso='+curso;
            $.get(url, function(dataReturn) {
              $('#load_frequencia').html(dataReturn);
            });
          }
        }
        </script> 
        
      <p style="font-size: 30px">Para visualizar o boletim selecione uma disciplina!</p>
       
          <form method="get" action="visualizar_boletins.php">
             <?php include_once '../../model/conn.php';
                $id = $_SESSION['id'];
                     $result5 = $conn -> query("SELECT id, id_disciplina FROM ministra WHERE id_professor='{$id}'");
                     $row5 = $result5->fetch(PDO::FETCH_OBJ);
                     
              ?>

           
          <select style="color: #000; width:300px; height: 30px; width: 50%; font-size: 17px;" id="disciplina" name="id_curso" class="form-text text-muted" onchange="buscar_minicursos()" required>
                  <?php include_once '../../model/conn.php';
                $id = $_SESSION['id'];
           ...
           <button style="width: 50%; font-size: 20px"  class="btn btn-primary" type="submit" >Boletins <i class="fa fa-angle-double-right" aria-hidden="true"></i></button>
           </form>
