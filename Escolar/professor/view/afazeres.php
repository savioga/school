    ...
    <section class="content-header">
      ...
      <?php  
        if(isset($_GET['res'])){
          $s = $_GET['res'];
          if($s=='sucesso'){
            echo '</i><div style="width: 70%;  padding: 6px; font-size: 30px" class="alert alert-success" role="alert">
      ...
      ?> 
      <form method="post" action="../model/salvar_afazeres.php">
        <div class="form-group">
         
          <br>
          <label style="font-size: 20px; margin-left: 5px" for="exampleInputEmail1">Afazer*</label>
          <input style="width: 50%; font-size: 20px; margin-left: 5px;" type="text" class="form-control" name="afazer" placeholder="Afazer" id="nome" required>
          <div id="resposta"></div>
          <br>
          <label style="font-size: 20px; margin-left: 5px; " for="exampleInputEmail1">Data de entrega/Realização*</label>
          <input style="width: 50%; font-size: 19px; margin-left: 5px ;" type="date" class="form-control" name="data" placeholder="Data" id="nome" required>
          <div id="resposta"></div>
          
          <br>
          <label style="font-size: 20px; margin-left: 5px" for="exampleInputEmail1">Selecione um Disciplina*</label><br>
        <select style="font-size: 20px; margin-left: 5px" id="Curso" name="id_ministra" class="form-text text-muted" required>
                <?php include_once '../../model/conn.php';
                $id = $_SESSION['id'];
                     $result = $conn -> query("SELECT id, id_disciplina FROM ministra WHERE id_professor='{$id}'");
                echo '<option style="color: black;" value="">Selecione Uma Disciplina </option>';
                      if ($result) {
                        while($row = $result->fetch(PDO::FETCH_OBJ)){
                           $result2 = $conn -> query("SELECT nome FROM disciplina WHERE id ='{$row-> id_disciplina}'");
                           $row2 = $result2->fetch(PDO::FETCH_OBJ);
                          echo '<option style="color: black; " value="'.$row -> id.'">'.$row2-> nome.'</option>';
                        }
                      }   
                ?>
        ...
        <button style="width: 50% ; font-size: 20px;  margin-left: 5px" id="button" onclick="return confirm('Deseja Salvar Afazeres')" class="btn btn-primary">Salvar</button>
      </form>
    <!-- Main content -->
